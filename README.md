...
`CTX_LAB_ELECTRON_FLOW_SCREENSHOT=dosya.png` ortam değişkeni verilirse son ekran görüntüsü ayrıca PNG artefaktı olarak yazılır; `CTX_LAB_ELECTRON_FLOW_SCREENSHOT_METRICS=metrik.json` verilirse aynı ekrandan boyut, renk kovası ve kontrast aralığı metrikleri JSON olarak üretilir. GitHub Actions Windows `desktop-verify` işi bu görüntüyü ve metrik dosyasını CI artefaktı olarak saklar.
...
Bu komut aynı repo erişimi, dal, `config.yaml`, beş hafıza klasörü ve geçici `archive/.ctxlab-write-test` yazma/silme kontrolünü çalıştırır. Token ekrana yazdırılmaz. Gerekli ortam değişkenleri yoksa komut ağ çağrısına çıkmadan hangi değerlerin eksik olduğunu söyler ve CI akışını kırmaz.
...
Mevcut bir ayna clone'u durum ekranında ve yenileme öncesinde `origin` GitHub reposu beklenen `owner/repo` ile karşılaştırılır; farklı bir repoya bağlı clone tespit edilirse eski indeks hazır sayılmaz ve fetch/pull yapılmadan Türkçe hata gösterilir.
...
`İş Akışı` ekranında seçili iş hattı için bağlı oturumlar, karar kayıtları, güncel durum, sıradaki adım ve çalışma kuralları tek bir devam brifinde birleştirilir. `Devam Brifi`, kaynak iş hattını veya oturum kaydını ve hedef aracı (`Codex` ya da `Claude Code`) açıkça seçtirir. Bu metin doğrudan kopyalanabilir, Markdown olarak indirilebilir, Claude hedefindeyken Claude'da yeni sohbet olarak açılabilir veya `handoffs/` altına kaydedilebilir. Aynı kaynak kayıt için kaydedilmiş son devam brifleri ekranda görünür kalır. Amaç, temiz bir Codex/Claude oturumunda sohbet geçmişi kaybolsa bile insan çalışma bağlamını hızlı geri yüklemektir.
Kaydedilen devam brifleri kaynak proje/repo bilgisini taşır ve `Proje Çalışma Merkezi` zaman akışında `Devam brifleri` filtresiyle izlenir.
`Günlük Devam Brifi` ekranı tüm açık iş hatlarını, engelli/bekleyen işleri ve işleme bekleyen kayıtları tek metinde toplar; bu brif kopyalanabilir, devam brifi olarak kaydedilebilir veya desteklenen masaüstü ortamlarında Türkçe Web Speech ile sesli okunabilir.
...
Sol proje listesinden bir proje seçildiğinde Oturum Akışı, İş Akışı, Karar Defteri ve devam brifi kaynakları aynı proje kapsamına daralır; kapsam etiketi `Tüm projeleri göster` ile temizlenebilir.
İş hattının `Sonraki Adım` alanı panodan güncellenebilir; devam brifi bu güncel değeri kullanır.
`Karar Defteri` ekranındaki `Yeni Karar` akışı, oturum geçmişinden bağımsız kaynaklı karar kaydı oluşturur ve seçili iş hattına bağlayabilir. Karar detayında `Karar Etkisi` alanı bağlı iş hattı, bağlı oturum ve kararın kaç gündür etkin olduğunu gösterir.
