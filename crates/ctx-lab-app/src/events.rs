    ...
    match event_type {
        "session_started" => {
            if let (Some(pid), Some(sid)) = (project_id, session_id) {
                let session_path = find_session_file(ctx_lab_dir, pid, sid);
                if let Some(path) = session_path {
                    crate::reconcile::incremental_update(conn, &path, ctx_lab_dir)?;
                }
            }
        }
        "session_ended" => {
...
/// Locate the session JSON file for a given project/session inside ctx_lab_dir.
///
...
fn find_session_file(ctx_lab_dir: &Path, project_id: &str, session_id: &str) -> Option<std::path::PathBuf> {
    ...
    for entry in entries.flatten() {
        ...
        if let Ok(meta_content) = fs::read_to_string(&meta_path) {
            if let Ok(meta) = toml::from_str::<ctx_lab_core::models::ProjectMeta>(&meta_content) {
...
mod tests {
    ...
    /// Set up a temp ctx-lab directory with DB, a project, and a session file.
