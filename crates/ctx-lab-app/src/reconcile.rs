...
use ctx_lab_core::models::{MachineProfile, ProjectMeta, Session};
use ctx_lab_core::roadmap::{self, ItemStatus};
...
pub fn full_rebuild(conn: &Connection, ctx_lab_dir: &Path) -> Result<ReconcileReport> {
    ...
    let machines_dir = ctx_lab_dir.join("machines");
...
mod tests {
    ...
    /// Create a temp directory with ctx-lab structure, an initialized DB, and
