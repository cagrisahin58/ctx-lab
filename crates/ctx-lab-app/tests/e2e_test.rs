//! End-to-end integration tests for ctx-lab-app.
//!
...
use ctx_lab_app::db;
use ctx_lab_app::events;
use ctx_lab_app::reconcile;

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------

/// Create a minimal ctx-lab directory structure inside `root` with one project.
///
/// Layout:
/// ```text
/// root/
...
fn test_rebuild_cache_via_commands() {
    ...
    // real ~/.ctx-lab/ directory, so we bypass it for testing).
    {
    ...
    let projects = ctx_lab_app::commands::get_projects_inner(&pool).unwrap();
    ...
    let sessions =
        ctx_lab_app::commands::get_sessions_inner(&pool, "e2e_proj".into(), 10).unwrap();
    ...
    let roadmap = ctx_lab_app::commands::get_roadmap_inner(&pool, "e2e_proj".into()).unwrap();
...
fn test_roadmap_shows_in_project_detail() {
    ...
    let detail =
        ctx_lab_app::commands::get_project_detail_inner(&pool, "e2e_proj".into()).unwrap();
