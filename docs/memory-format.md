...
Yeni `codex_run` memory kayıtları genel memory sözleşmesiyle uyumlu kalmak için `status: active` taşır; gerçek koşu durumu `run_status` alanında (`dry_run`, `running`, `succeeded`, `failed`, `blocked`) tutulur. Sağlık denetimi yeni kayıtlar için `run_status` alanını zorunlu ve bu değer listesiyle sınırlı kabul eder. Eski `status: dry_run` gibi run-status değerleri geriye uyumlu okunur.
